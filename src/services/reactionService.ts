 ...
 */
export async function removeReactionRole(messageId: bigint, reaction: string): Promise<boolean> {
  try {
 ...
 */
export async function removeAllReactionRoles(messageId: bigint): Promise<number> {
  try {
    const result = await prisma.reaction_role.deleteMany({
 ...
 */
export async function getReactionRoles(messageId: bigint): Promise<reaction_role[]> {
 ...
 */
export async function getReactionRolesForMessage(messageId: bigint): Promise<reaction_role[]> {
  return getReactionRoles(messageId);
}

/**
 * Check if a message has any reaction roles
 */
export async function hasReactionRoles(messageId: bigint): Promise<boolean> {
