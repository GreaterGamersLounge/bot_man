...
export const prefixCommands: PrefixCommand[] = [
  {
    ...
    async execute(message: Message, args: string[]): Promise<void> {
      ...
      if (args.length < 3) {
        await message.reply('Usage: !addreactionrole <message_id> :emoji: <role_id>');
        return;
      }

      const [messageId, emoji, roleId] = args as [string, string, string];
      const channel = message.channel as TextChannel;

      // Validate message exists
      const targetMessage = await channel.messages.fetch(messageId).catch(() => null);
      if (!targetMessage) {
        await message.reply('Please supply a message_id from this channel');
        return;
      }
      ...
      if (!role) {
        await message.reply('Please supply a role_id from this server');
        return;
      }

      // Validate emoji
      if (!isEmojiInServer(message.guild.emojis.cache, emoji)) {
        await message.reply('Please supply an emoji from this server');
        return;
      }
      ...
      const reactionKey = getReactionKey(emoji);
      try {
        await targetMessage.react(reactionKey);
      } catch {
        await message.reply('Failed to add reaction. Make sure the emoji is valid.');
        return;
      }
      ...
      const messageUrl = discordUrl(message.guild.id, channel.id, messageId);

      const embed = new EmbedBuilder()
        .setColor(EMBED_COLOR)
        .setDescription(`<@&${roleId}> successfully linked to ${emoji} for [this message](${messageUrl})`);
  ...
  },
  {
    ...
    async execute(message: Message, args: string[]): Promise<void> {
      ...
      if (args.length < 2) {
        await message.reply('Usage: !removereactionrole <message_id> :emoji:');
        return;
      }

      const [messageId, emoji] = args as [string, string];
      const channel = message.channel as TextChannel;
      ...
      if (!deleted) {
        await message.reply('Not a valid message/emoji combination.');
        return;
      }
      ...
      try {
        const reactionKey = getReactionKey(emoji);
        const reaction = targetMessage.reactions.cache.get(reactionKey);
        if (reaction) {
          await reaction.remove();
        }
      } catch {
  ...
  },
  {
    ...
    async execute(message: Message, args: string[]): Promise<void> {
      ...
      if (args.length < 1) {
        await message.reply('Usage: !removeallreactionroles <message_id>');
        return;
      }
      ...
      if (reactionRoles.length === 0) {
        await message.reply('There are no reaction roles on that message');
        return;
      }
